...
%OUTPUT ARGUMENTS:
%None, but the variable "fastq_seqs" is appended to a MAT file called
...
for f=1:d %loop through each FASTQ file.
    ...
    for ii=1:blocks_minus1 %loop through each block, except for the last block.
        ...
        idx=1;
        for j=a:b
            fastq_seqs{j}=data{idx}; %save the entry Header.
            idx=idx+1;
        end
    ...
    save('fastq_analysis.mat','fastq_seqs','-append','-v7.3'); %save the variable. The '-v7.3' flag is needed to save variables >2GB.
